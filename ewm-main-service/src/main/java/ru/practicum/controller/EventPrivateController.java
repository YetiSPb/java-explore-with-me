...
public class EventPrivateController {
    ...
    public List<EventShortDto> searchAllEvents(@PathVariable Long userId,
                                               @PositiveOrZero @RequestParam(defaultValue = "0") Integer from,
                                               @Positive @RequestParam(defaultValue = "10") Integer size) {
        log.info("Received GET request for all categories from {} to {}", from, size);
        return eventService.searchAllEvents(userId, from, size);
    }

    @GetMapping("/{userId}/events/{eventId}")
    @ResponseStatus(HttpStatus.OK)
    public EventFullDto searchById(@PathVariable Long userId, @PathVariable Long eventId) {
        log.info("Received GET request to find an event by id {} from user by id {}", eventId, userId);
        return eventService.searchById(userId, eventId);
    }
    ...
    @ResponseStatus(HttpStatus.OK)
    public List<ParticipationRequestDto> searchParticipationRequests(@PathVariable Long userId,
                                                                     @PathVariable Long eventId) {
        log.info("Received GET request to find participant requests for the event by id {} by current user by id {}",
                eventId, userId);
        return eventService.searchParticipationRequests(userId, eventId);
    }
