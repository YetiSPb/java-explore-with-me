...
public class CompilationController {
    ...
    public List<CompilationDto> searchCompilations(@RequestParam(required = false) Boolean pinned,
        ...
        return compilationService.searchCompilations(pinned, page);
    }

    @GetMapping("/{compId}")
    @ResponseStatus(HttpStatus.OK)
    public CompilationDto searchById(@PathVariable Long compId) {
        log.info("Received GET request to find compilation by id {}", compId);
        return compilationService.searchById(compId);
    }
}
